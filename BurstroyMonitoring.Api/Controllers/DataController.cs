...
namespace BurstroyMonitoring.Api.Controllers
{
    ...
    public class DataController : ControllerBase
    {
        ...
        [HttpGet("sensors/{sensorId:int}/dov")]
        public async Task<IActionResult> GetDOVData(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Error loading DOV data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке данных" });
            }
        }

        [HttpGet("sensors/{sensorId:int}/dspd")]
        public async Task<IActionResult> GetDSPDData(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
                ...
                using (var connection = new NpgsqlConnection(connectionString))
                {
                    using (var command = new NpgsqlCommand(query, connection))
                    {
                        ...
                        using (var reader = await command.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                measurements.Add(new DSPDMeasurementViewModel
                                {
                                    ReceivedAt = reader.GetDateTime(reader.GetOrdinal("ReceivedAt")),
            ...
            {
                _logger.LogError(ex, "Error loading DSPD data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке данных" });
            }
        }

        [HttpGet("sensors/{sensorId:int}/dust")]
        public async Task<IActionResult> GetDUSTData(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
                ...
                using (var connection = new NpgsqlConnection(connectionString))
                {
                    using (var command = new NpgsqlCommand(query, connection))
                    {
                        ...
                        using (var reader = await command.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                measurements.Add(new DUSTMeasurementViewModel
                                {
                                    ReceivedAt = reader.GetDateTime(reader.GetOrdinal("ReceivedAt")),
            ...
            {
                _logger.LogError(ex, "Error loading DUST data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке данных" });
            }
        }

        [HttpGet("sensors/{sensorId:int}/iws")]
        public async Task<IActionResult> GetIWSData(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
                ...
                using (var connection = new NpgsqlConnection(connectionString))
                {
                    using (var command = new NpgsqlCommand(query, connection))
                    {
                        ...
                        using (var reader = await command.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                measurements.Add(new IWSMeasurementViewModel
                                {
                                    ReceivedAt = reader.GetDateTime(reader.GetOrdinal("ReceivedAt")),
            ...
            {
                _logger.LogError(ex, "Error loading IWS data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке данных" });
            }
        }

        [HttpGet("sensors/{sensorId:int}/mueks")]
        public async Task<IActionResult> GetMUEKSData(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
                ...
                using (var connection = new NpgsqlConnection(connectionString))
                {
                    using (var command = new NpgsqlCommand(query, connection))
                    {
                        ...
                        using (var reader = await command.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                measurements.Add(new MUEKSMeasurementViewModel
                                {
                                    ReceivedAt = reader.GetDateTime(reader.GetOrdinal("ReceivedAt")),
            ...
            {
                _logger.LogError(ex, "Error loading MUEKS data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке данных" });
            }
        }
        ...
        [HttpGet("sensors/{sensorId:int}/dov/hourly")]
        public async Task<IActionResult> GetDOVDataHour(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
            ...
            {
                _logger.LogError(ex, "Error loading hourly DOV data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке почасовых данных" });
            }
        }

        [HttpGet("sensors/{sensorId:int}/dspd/hourly")]
        public async Task<IActionResult> GetDSPDDataHour(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
            ...
            {
                _logger.LogError(ex, "Error loading hourly DSPD data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке почасовых данных" });
            }
        }

        [HttpGet("sensors/{sensorId:int}/dust/hourly")]
        public async Task<IActionResult> GetDUSTDataHour(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
            ...
            {
                _logger.LogError(ex, "Error loading hourly DUST data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке почасовых данных" });
            }
        }

        [HttpGet("sensors/{sensorId:int}/iws/hourly")]
        public async Task<IActionResult> GetIWSDataHour(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
            ...
            {
                _logger.LogError(ex, "Error loading hourly IWS data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке почасовых данных" });
            }
        }

        [HttpGet("sensors/{sensorId:int}/mueks/hourly")]
        public async Task<IActionResult> GetMUEKSDataHour(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
            ...
            {
                _logger.LogError(ex, "Error loading hourly MUEKS data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке почасовых данных" });
            }
        }
        ...
        [HttpGet("sensors/{sensorId:int}/dov/ten-minute")]
        public async Task<IActionResult> GetDOVDataTenMinuteInterval(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
            ...
            {
                _logger.LogError(ex, "Error loading 10-minute DOV data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке данных с 10-минутным интервалом" });
            }
        }

        [HttpGet("sensors/{sensorId:int}/dspd/ten-minute")]
        public async Task<IActionResult> GetDSPDDataTenMinuteInterval(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
            ...
            {
                _logger.LogError(ex, "Error loading 10-minute DSPD data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке данных с 10-минутным интервалом" });
            }
        }

        [HttpGet("sensors/{sensorId:int}/dust/ten-minute")]
        public async Task<IActionResult> GetDUSTDataTenMinuteInterval(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
            ...
            {
                _logger.LogError(ex, "Error loading 10-minute DUST data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке данных с 10-минутным интервалом" });
            }
        }

        [HttpGet("sensors/{sensorId:int}/iws/ten-minute")]
        public async Task<IActionResult> GetIWSDataTenMinuteInterval(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
            ...
            {
                _logger.LogError(ex, "Error loading 10-minute IWS data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке данных с 10-минутным интервалом" });
            }
        }

        [HttpGet("sensors/{sensorId:int}/mueks/ten-minute")]
        public async Task<IActionResult> GetMUEKSDataTenMinuteInterval(int sensorId, [FromQuery] int days = 1)
        {
            try
            {
                var fromDate = DateTime.UtcNow.AddDays(-days);
            ...
            {
                _logger.LogError(ex, "Error loading 10-minute MUEKS data for sensor {SensorId}", sensorId);
                return StatusCode(500, new { error = "Ошибка при загрузке данных с 10-минутным интервалом" });
            }
        }
