...
if systemctl is-active --quiet burstroy-monitoring-worker; then
    echo "Stopping burstroy-worker service..."
    systemctl stop burstroy-monitoring-worker
fi
...
echo "You can now start the service: sudo systemctl start burstroy-monitoring-worker"
